...
$dbh = DBI->connect( "dbi:SQLite:dbname=../../../db/2014examPredict.sqlite" ) || die "Cannot connect: $DBI::errstr";
...
#		$column[19] =~ s/總/國+英+數+自+社/;
		...
		$column[21] =~ s/^(-|－|—|0|不足額|無超額|)$/NULL/; #超額篩選
		$column[21] =~ s/<|u//;	#超額篩選
		$column[22] =~ s/^$/0/;	#二階段錄取人數
		$column[23] =~ s/^$/0/;	#備取人數
		$column[24] =~ s/.*(\d+).*/$1/;	# 分發最低標準
		$column[24] =~ s/正取|IE取|無|備|-|^$/0/;
